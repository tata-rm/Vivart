...
enviarPost.onclick = async function(event) {
    ...
    const storedAccount = localStorage.getItem('usuarioOportuniza');
    ...
    dadosPost.append('cnpjPost', cnpjPost);
...
async function getPost(event) {
    ...
    let idPost = localStorage.getItem('cnpjUser');
...
async function enviarImgPerfilOportuniza() {
    ...
    if (file) {
        ...
        formData.append('cnpjUser', localStorage.getItem('cnpjUser'))

        const response = await fetch('http://localhost:3005/api/atualizarImgPerfilOportuniza', {
            method: 'POST',
            body: formData
        });
...
uploadFoto.addEventListener('change', enviarImgPerfilOportuniza);

/*---------------------------------------------------*/

async function getPerfilOportuniza(event) {

    let cnpj = localStorage.getItem('cnpjUser');

    let dataOportuniza = {cnpj}

    const response = await fetch(`http://localhost:3005/api/getOportuniza/${dataOportuniza.cnpj}`, {
        method: "GET"
    });
    ...
    if(result.success) {
        ...
        if(result.dataOportuniza.fotoPerfil) {
            let url = "http://localhost:3005/images/";
            document.getElementById('img_perfil').src = url + result.dataOportuniza.fotoPerfil
        }

        let nome = 
        `<div id="info">
                <p id="nome">${result.data.nome}</p>
                <p>${result.dataOportuniza.email}</p>
                <p>${result.dataOportuniza.area}</p>
                <p>${result.dataOportuniza.texto}</p>
        </div>`;
...
window.onload = getPerfilOportuniza();